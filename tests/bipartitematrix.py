...
def adjlist_to_bipartite(network):
    ...
    idMap0 = dict(list(zip([node["id"] for node in network["nodes"] if node["bipartite"] == 0], list(range(n)))))
    idMap1 = dict(list(zip([node["id"] for node in network["nodes"] if node["bipartite"] == 1], list(range(m)))))
...
def write_bipartite_matrix(fh, m, bpgraph):
    ...
    for i in range(len(bpgraph)):
        for j in range(m):
            if j in bpgraph[i] and bpgraph[i][j] == 1:
                fh.write("1")
            else:
                fh.write("0")
            if j < m-1:
...
#     # make dict id:i mapping ids to 0-based enumeratino for the two clases
#     idMap0 = dict(zip([node["name"] for node in graph["nodes"] if node["attributes"]["bipartite"] == 0], range(n)))
#     idMap1 = dict(zip([node["name"] for node in graph["nodes"] if node["attributes"]["bipartite"] == 1], range(m)))
