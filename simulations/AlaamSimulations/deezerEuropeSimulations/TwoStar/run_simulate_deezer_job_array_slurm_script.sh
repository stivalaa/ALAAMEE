...
#SBATCH --output=alaam_sim_deezer-%A_%a.out
#SBATCH --error=alaam_sim_deezer-%A_%a.err
#SBATCH --ntasks=1
#SBATCH --mem=500MB
#SBATCH --array=0-200
...
SIMSTATS_FILE=stats_sim_deezer_${SLURM_ARRAY_TASK_ID}.txt
...
time ./runALAAMsimulateDeezer.py ${SLURM_ARRAY_TASK_ID} > ${SIMSTATS_FILE}
