...
#SBATCH --output=alaam_sim_higgs-%A_%a.out
#SBATCH --error=alaam_sim_higgs-%A_%a.err
#SBATCH --ntasks=1
#SBATCH --mem=3GB
#SBATCH --array=0-200
...
SIMSTATS_FILE=stats_sim_higgs_${SLURM_ARRAY_TASK_ID}.txt
...
time ./runALAAMsimulateHiggs.py ${SLURM_ARRAY_TASK_ID} > ${SIMSTATS_FILE}
