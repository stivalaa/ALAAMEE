#!/usr/bin/env python3
#
# File:    run runALAAMsimulateLattice.py
# Author:  Alex Stivala
# Created: July 2023
#
"""Simulate from  Autologistic Actor Attribute Model (ALAAM).

   Usage: runALAAMsimulateLattice.py TASK_ID
...
param_func_list =      [changeDensity, partial(changeGWContagion, log(2.0)), changeActivity, changeContagion]
labels =               ["Density",     "GWContagion",                        "Activity",     "Contagion"]

theta =       np.array([-0.20,         theta_gwcontagion,                    -0.05,          0])
...
simulate_from_network_attr(
    '../../../latticeSimulations/data/lattice.net',
    param_func_list,
    labels, 
    theta,
    sampler_func = basicALAAMsampler,
