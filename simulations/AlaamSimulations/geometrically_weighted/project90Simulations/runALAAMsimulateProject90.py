#!/usr/bin/env python3
#
# File:    run runALAAMsimulateProject90.py
# Author:  Alex Stivala
# Created: July 2023
#
"""Simulate from  Autologistic Actor Attribute Model (ALAAM).

   Usage: runALAAMsimulateProject90.py TASK_ID

   where TASK_ID is supplied by SLURM_ARRAY_TASK_ID in the range
   0..300 and we here do (TASK_ID - 100)/100 to get in interval [-1.0, 2.0] 
"""
import sys
...
theta_gwcontagion = (TASK_ID - 100) / 100    

# parameters and corresponding labels 
param_func_list =      [changeDensity, changeActivity,  partial(changeGWContagion, log(2.0)), partial(changeoOb, "binattr"), partial(changeoOc, "contattr"), changeContagion]
labels =               ["Density",    "Activity" ,      "GWContagion",                        "binattr_oOb",                "contattr_oOc",                  "Contagion"]

theta =       np.array([-15.0,         0.550,           theta_gwcontagion,                    1.20,                          1.15,                           0])
...
simulate_from_network_attr(
    '../../project90Simulations/data/project90_giantcomponent_edgelist.txt',
    param_func_list,
    labels, 
    theta,
    binattr_filename = '../../project90Simulations/data/binaryAttribute_50_50_n4430_with_header.txt',
    contattr_filename = '../../project90Simulations/data//continuousAttributes_n4430_with_header.txt',
    sampler_func = basicALAAMsampler,
