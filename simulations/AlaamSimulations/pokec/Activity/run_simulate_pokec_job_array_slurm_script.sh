...
#SBATCH --output=alaam_sim_pokec-%A_%a.out
#SBATCH --error=alaam_sim_pokec-%A_%a.err
#SBATCH --ntasks=1
#SBATCH --mem=4GB
#SBATCH --array=0-200
...
SIMSTATS_FILE=stats_sim_pokec_${SLURM_ARRAY_TASK_ID}.txt
...
time ./runALAAMsimulatePokec.py   ${SLURM_ARRAY_TASK_ID} > ${SIMSTATS_FILE}
