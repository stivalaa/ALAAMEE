...
#SBATCH --output=alaam_sim_lattice-%A_%a.out
#SBATCH --error=alaam_sim_lattice-%A_%a.err
#SBATCH --ntasks=1
#SBATCH --mem=500MB
#SBATCH --array=0-200
...
SIMSTATS_FILE=stats_sim_lattice_${SLURM_ARRAY_TASK_ID}.txt
...
time ./runALAAMsimulateLattice.py ${SLURM_ARRAY_TASK_ID} > ${SIMSTATS_FILE}
