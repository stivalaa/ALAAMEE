...
##   model12.tex
##   model12.txt
##   model12_siena_gof_degree.eps
##   model12_siena_gof_geodesic.eps
##   model12_siena_gof_behavior.eps
##
...
model12data <- sienaDataCreate(contact, sex, Wound_status, Tumour_dep, Mating_season)

print01Report(data = model12data, modelname="model12")

model12effects<-getEffects(model12data)
model12effects <- includeEffects(model12effects, RateX, type="rate", interaction1="sex")
model12effects <- includeEffects(model12effects, RateX, type="rate", interaction1="Tumour_dep")
model12effects <- includeEffects(model12effects, RateX, type="rate", interaction1="Wound_status")
## Note using special effects for non-directed networks (RSiena manual s. 12.1.2)
model12effects <- includeEffects(model12effects, egoPlusAltX, interaction1="sex") ## egoPlusAltSqX for sex is too correlated with degree(density) so dropped
model12effects <- includeEffects(model12effects, egoPlusAltX, egoPlusAltSqX,interaction1="Wound_status")
model12effects <- includeEffects(model12effects, egoPlusAltX, egoPlusAltSqX,interaction1="Tumour_dep")
## Model 04 adds degPlus and transTriads to Model 03
## (Model 03 has bad gof on geodesic distance and egree)
model12effects <- includeEffects(model12effects, degPlus)
model12effects <- includeEffects(model12effects, transTriads)
...
#model12effects <- includeEffects(model12effects, RateX, type="rate", interaction1="Mating_season")
model12effects <- includeEffects(model12effects, egoX, interaction1="Mating_season")
## does not converge (not pos def covariancce matrix):
#model12effects <- includeEffects(model12effects, name = "Tumour_dep", effFrom, interaction1 = "Mating_season")

## Model 11 adds monadic covariate effects for Tumour grade "behaviour" variable
model12effects <- includeEffects(model12effects, name = "Tumour_dep", effFrom, interaction1 = "sex")
model12effects <- includeEffects(model12effects, name = "Tumour_dep", effFrom, interaction1 = "Wound_status")

## Model 12 adds effect for alter new wound counts
model12effects <- includeEffects(model12effects, name = "Tumour_dep", totXAlt, interaction1 = "Wound_status", interaction2 = "contact")
...
model12alg <- sienaAlgorithmCreate(cond=TRUE, projname="model12")


## Estimate model
model12 <- siena07(model12alg, batch=TRUE, data=model12data, effects=model12effects, returnDeps=TRUE)

print(summary(model12))

## Output HTML and LaTeX tables
siena.table(model12, type='html', sig=TRUE)
siena.table(model12, type='tex', sig=TRUE)

## Goodness-of-fit
model12gof_behavior <- sienaGOF(model12, BehaviorDistribution, varName="Tumour_dep")
print(model12gof_behavior)
model12gof_degree <- sienaGOF(model12, OutdegreeDistribution, varName="contact")
print(model12gof_degree)
model12gof_geodesic <- sienaGOF(model12, GeodesicDistribution, varName="contact")
print(model12gof_geodesic)
...
##par(mfrow=c(3,1))
postscript('model12_siena_gof_degree.eps')
plot(model12gof_degree)
dev.off()
postscript('model12_siena_gof_geodesic.eps')
plot(model12gof_geodesic)
dev.off()
postscript('model12_siena_gof_behavior.eps')
plot(model12gof_behavior)
dev.off()
