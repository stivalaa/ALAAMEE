...
#SBATCH --job-name="stochastic_approx_alaam_nostruct"
#SBATCH --nodes=1
#SBATCH --cpus-per-task=1
#SBATCH --mem-per-cpu=500MB
#SBATCH --time=0-01:00:00
#SBATCH --output=alaam_sa_seierstad_nostruct-%j.out
#SBATCH --error=alaam_sa_seierstad_nostruct-%j.err
...
ESTIM_FILE=estimation_sa_seierstad_directors_bipartite_nostruct.txt

if [ -f ${ESTIM_FILE} ]; then
  mv ${ESTIM_FILE} ${ESTIM_FILE}.OLD
fi

time python3 ./runALAAMSASeierstad_nostruct.py | tee ${ESTIM_FILE}
