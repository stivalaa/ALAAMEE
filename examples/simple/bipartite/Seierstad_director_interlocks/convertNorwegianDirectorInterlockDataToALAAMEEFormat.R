#!/usr/bin/env Rscript
##
## File:    convertNorwegianDirectorInterlockDataToALAAMEEFormat.R
## Auithor: Alex Stivala
## Created: May 2024
##
## (Based on convertNorwegianDirectorInterlockDataToEstimNetDirectedFormat.R
## April 2024).
##
## Read the Seierstad & Opsahl (2011) Norwegian director interlock
## data and convert to format for ALAAMEE. Use binary indictor for female
## gender on mode A (persons) as 'outcome' attribute for ALAAM, and
## add centrality measures for nodes.
##
## Citation for data:
##
...
##
## Usage:
##    Rscript convertNorwegianDirectorInterlockDataToALAAMEEFormat.R datadir
...
write.table(outcome, 'norwegian_director_interlock_20090801_outcome.txt',
            row.names=FALSE, col.names=TRUE, quote=FALSE)
...
system.time( contattr <- data.frame(betweenness = betweenness(g,
                                                          directed = FALSE, 
                                                          normalized = FALSE)) )
...
write.table(contattr, 'norwegian_director_interlock_20090801_contattr.txt',
            row.names=FALSE, col.names=TRUE, quote=FALSE)
