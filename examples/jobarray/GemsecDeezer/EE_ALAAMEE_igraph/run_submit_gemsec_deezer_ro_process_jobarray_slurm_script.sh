...
jobid=$(sbatch --parsable run_gemsec_deezer_ro_jobarray_slurm_script.sh)
echo ${jobid}
R_jobid=$(sbatch --parsable --dependency=afterok:${jobid} run_gemsec_deezer_ro_R_scripts_slurm_script.sh)
echo ${R_jobid}
sbatch --dependency=afterok:${R_jobid} run_simulate_gof_gemsec_deezer_ro_slurm_script.sh
