#!/usr/bin/Rscript
##
## File:    computeALAMEEcovariance_var_total_runs_faster.R
## Author:  Alex Stivala
## Created: July 2024
##
## NOTE:
## Copy of computeEstimNetDirectedCovariance.R, but changed so instead
## of --max_runs option, it reads the theta and dzA files first, and
## then iterates over 1..maxruns. This is for journal revision where 
## plot of maxruns higher than orignal 100 requested, and turns out that
## with 500 it was impractically slow to do it the existing way which
## involved the iteration in a shell script calling running
## computeEstimNetDirectedCovariance.R --max_runs=$i each iteration
## which was too slow. This script is just for this purpose, not 
## generally useful (unlike computeEstimNetDirectedCovariance.R).
## END NOTE.
##
##
## Read theta parameter MCMC estimates and simulated statistcs from
## ALAAMEE output and use them to estimate parameter values
## and standard errors.
##
...
##
##
## Usage: Rscript computeALAMEEcovariance.R thetaPrefix dzAprefix
