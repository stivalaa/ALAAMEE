...
if (length(args) > 1) {
  cat("Usage: convertSNAPpokecToALAAMEEFormat.R [maxdegree]\n")
  quit(save="no")
}
...
##
catattr <- pokec[, c("gender", "region", "relation_to_smoking")]  # also make categorical gender
## https://snap.stanford.edu/data/soc-pokec-readme.txt
## region:
...
##V(g)$relation_to_smoking <- catattr$relation_to_smoking


## TODO convert relation_to_smoking to a binary attribute
### But this field is free-form text with a huge number of responses
### so need to parse Slokak to do it...
###catattr$relation_to_smoking <- factor(catattr$relation_to_smoking)
###print(levels(catattr$relation_to_smoking))
...
outcomebinattr <- data.frame(public = V(g)$public)
summary(outcomebinattr$public)
write.table(outcomebinattr, file = "soc-pokec-outcome.txt",
